...
var mkProjectDir = function (pageUrl) {
    var projectName = path.basename(path.basename(pageUrl, '.html'), '.htm');
...
var publishOnePage = function (pageUrl, allPackName) {
    ...
    var projectRoot = allPackName || path.basename(path.basename(pageUrl, '.html'), '.htm');
    var pageName = path.basename(path.basename(pageUrl, '.html'), '.htm');
    fs.writeFileSync(path.resolve(cwd, './__publish__/' + projectRoot + '/' + pageName + '.htm'), analyzeHtml(pageUrl, projectRoot, pageName));
...
exports.publish = function (pageUrl) {
    ...
    if (pageUrl === allPackName) {
        ...
        files.forEach(function (value) {
            if (path.extname(value) === '.html' || path.extname(value) === '.htm') {
