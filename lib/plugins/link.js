...
exports.dealWithUrl = function (csscode, cssPath, projectPublishRoot) {
    return csscode.replace(utils.regs['url'], function (mu, $1, $2, $3, $4) {
        ...
        if (!!config.extractTo) {
            ...
            var fileExtractTo = path.resolve(path.dirname(staticPath).replace(cwd, extractToBase));
            shell.mkdir('-p', fileExtractTo);
            if (fs.existsSync(staticPath)) {
                shell.cp('-Rf', staticPath, fileExtractTo);
            } else {
                console.error('#red{[file not exist]}:' + staticPath + '\n');
            }
            console.info('#green{[extract file]} ' + $3 + '\n');
        }
...
exports.preReplace = function (m, htmlpath, projectRoot, pageName, concatServerBaseUrl, projectPublishRoot) {
    if (m.match(/role\s*=\s*(['"]?)\s*debug\s*\1/) !== null) { //只处理带有role=debug的项
        ...
        if (!concatedCss[cssPath]) {
            if (!!config.extractTo && cssPath.indexOf(path.resolve(cwd, config.template)) === -1) { //过滤template中的css, 其中的css都在抽取整个template时抽取出
                var extractToBase = path.resolve(cwd, './' + config.extractTo);
                var fileExtractTo = path.resolve(path.dirname(cssPath).replace(cwd, extractToBase));
                shell.mkdir('-p', fileExtractTo);
                if (fs.existsSync(cssPath)) {
                    shell.cp('-Rf', cssPath, fileExtractTo);
                } else {
                    console.error('#red{[file not exist]}:' + cssPath + '\n');
                }
            }
