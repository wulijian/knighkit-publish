 ...
 */
exports.preReplace = function (m, htmlpath, projectRoot, pageName, concatServerBaseUrl, projectPublishRoot, injs) {
    if (m.match(/role\s*=\s*(['"]?)\s*debug\s*\1/) !== null) { //只处理带有role=debug的项
        var getHref = m.match(/(?:\shref\s*=\s*)(['"]?)([^'"\s]*)\1/);
        var cssPath = path.resolve(path.dirname(htmlpath.replace(/pages\/|pages\\/g, '')), getHref[2]);
...
exports.replace = function (m, htmlpath, projectRoot, pageName, baseurl, publishroot) {
    ...
    var cssName = htmlpath.replace(path.join(cwd, 'pages/'), '').replace(cwd, '').replace(path.extname(htmlpath), '').replace(/\/|\\/g, '_');
    ...
    if (m.match(/role\s*=\s*(['"]?)\s*debug\s*\1/) !== null) {//设置为role=debug的标签，删掉
        if (!addedTohtml) {//最后一个link标签
            ...
            var cssMd5 = md5.update(mincss)
                .digest('hex')
                .substring(0, 6);
            fs.writeFileSync(projectRoot + '/styles/' + cssName + '_' + cssMd5 + '.css', mincss);
            allCss.forEach(function (val) {
                console.log(val.match(/(?:\shref\s*=\s*)(['"]?)([^'"\s]*)\1/)[2]);
            });
            console.info('#green{[pack css files done.]}\n');
            addedTohtml = true;
            return '<link rel="stylesheet" href="' + baseurl + '/styles/' + cssName + '_' + cssMd5 + '.css"/>';
        } else {
            return '';
        }
    }
