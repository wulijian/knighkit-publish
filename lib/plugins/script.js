...
var packJsCode = function (htmlpath, projectRoot, pageName, scriptmain, baseurl, publishroot) {
    ...
    var scriptDir = path.dirname(htmlpath.replace(/pages\/|pages\\/g, ''));
    ...
    fs.writeFileSync(path.resolve(cwd, projectRoot + '/scripts/' + scriptName + '_' + jsMd5 + '.js'), jsmincode);
    return jsMd5;
};
/**
 * 提前压缩
...
exports.replace = function (m, htmlpath, projectRoot, pageName, baseurl, publishroot) {
    ...
    var scriptName = htmlpath.replace(path.join(cwd, 'pages/'), '').replace(cwd, '').replace(path.extname(htmlpath), '').replace(/\/|\\/, '_');
    ...
    if (mainJs !== null) { //找到seajs的默认入口，压缩合并，并查找里面的img标签，拷贝图片，并md5
        return '<script type="text/javascript" charset="' + charset
            + '" data-mainentry="' + mainJs[2].replace(/^\.*/, '')
            + '" src="' + baseurl + '/scripts/' + scriptName + '_' + jsMd5 + '.js" async></script>'
