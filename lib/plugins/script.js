...
var packJsCode = function (htmlpath, projectRoot, pageName, scriptmain, baseurl, publishroot) {
    ...
    var jsfilePath = path.resolve(cwd, './__publish__/' + projectRoot + '/scripts/' + pageName + '.js');
    var jscode = fs.readFileSync(jsfilePath, 'utf-8');
    jscode = jscode.replace(require('./link').reg, function (m) {//处理js中出现的img标签
        require('./link').preReplace(m, htmlpath, projectRoot, pageName, baseurl, publishroot);
        return '';
    });
    ...
    fs.writeFileSync(path.resolve(cwd, './__publish__/' + projectRoot + '/scripts/' + pageName + '_' + jsMd5 + '.js'), jsmincode);
    return jsMd5;
};
/**
 * 提前压缩
